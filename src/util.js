...
const getModByDate = (mods, date, type) => {
  ...
  mods.filter((mod) => mod.date ? mod.date.isSame(date, type) : false)
    .forEach((_mod) => {
...
const explodeDateRanges = (mods) => {
  return mods.map(mod => {
    const diff = differenceInDays(mod.endDate, mod.startDate);

    if (!diff) { // if the diff is 0 just return the mod
      mod.date = mod.startDate
      return mod;
    }
